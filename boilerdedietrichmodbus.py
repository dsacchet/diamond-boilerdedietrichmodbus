...
class BoilerDeDietrichModbusCollector(diamond.collector.Collector):
	...
	##########
	# 14 => jour temperature jour setpoint A / 0 => 400 / Increment 1 / Units 0.1 deg C
	setpoint_day_room_temp_a = self.get_value(instrument,14,1,False)
	if setpoint_day_room_temp_a is not None:	
		self.publish('setpoint_day_room_temp_a',setpoint_day_room_temp_a, precision=1)

	##########
	# 15 => night room temperature setpoint A / 0 => 400 / Increment 1 / Units 0.1 deg C
	setpoint_night_room_temp_a = self.get_value(instrument,15,1,False)
	if setpoint_night_room_temp_a is not None:	
		self.publish('setpoint_night_room_temp_a',setpoint_night_room_temp_a, precision=1)

	##########
	# 16 => antifreeze room temperature setpoint A / 0 => 400 / Increment 1 / Units 0.1 deg C
	antifreeze_room_temp_setpoint_a = self.get_value(instrument,16,1,False)
	if antifreeze_room_temp_setpoint_a is not None:	
		self.publish('antifreeze_room_temp_setpoint_a',antifreeze_room_temp_setpoint_a, precision=1)
